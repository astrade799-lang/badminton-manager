...
const btnG = { padding: '7px 16px', borderRadius: 8, background: '#16a34a', color: 'white', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }
...
export default function Transaksi() {
  ...
  async function muatData() {
    const [resProduk, resHistory] = await Promise.all([
      ...
      supabase.from('transaksi_stok')
        .select('*, stok(nama, satuan_besar, satuan_kecil, isi_per_satuan)')
  ...
  // ── Produk yang dipilih (untuk preview) ──────────────────
  const produkJual = produkList.find(p => p.id === parseInt(jualProduk))
  const produkLap  = produkList.find(p => p.id === parseInt(lapProduk))

  // ── Kalkulasi Jual ────────────────────────────────────────
  const jualJmlPcs = produkJual
    ? (jualSatuan === 'besar'
        ? (parseInt(jualJumlah) || 0) * produkJual.isi_per_satuan
        : (parseInt(jualJumlah) || 0))
    : 0
  const jualHargaDefault = produkJual
    ? (jualSatuan === 'besar' ? produkJual.harga_jual_besar : produkJual.harga_jual_pcs)
    : 0
  ...
  async function simpanJual() {
    ...
    if (jualJmlPcs > produkJual.stok_pcs) {
      tampilPesan(`⚠️ Stok tidak cukup! Tersedia: ${tampilStok(produkJual.stok_pcs, produkJual.isi_per_satuan, produkJual.satuan_besar, produkJual.satuan_kecil)}`)
      return
    }
    setLoading(true)
    ...
    // Catat transaksi_stok
    await supabase.from('transaksi_stok').insert([{
      produk_id:     produkJual.id,
      tipe:          'jual',
      jumlah_pcs:    jualJmlPcs,
      harga_per_pcs: jualSatuan === 'besar'
        ? Math.round(jualHargaFinal / produkJual.isi_per_satuan)
        : jualHargaFinal,
      diskon:        jualDiskonNom,
      total:         jualTotal,
    ...
    if (jualTotal > 0) {
      await supabase.from('kas').insert([{
        jenis:        'masuk',
        kategori:     'Penjualan Stok',
        sub_kategori: produkJual.kategori,
  ...
  async function simpanLapangan() {
    ...
    if (!produkLap) return
    if (lapJmlPcs > produkLap.stok_pcs) {
      tampilPesan(`⚠️ Stok tidak cukup! Tersedia: ${tampilStok(produkLap.stok_pcs, produkLap.isi_per_satuan, produkLap.satuan_besar, produkLap.satuan_kecil)}`)
      return
    }
    setLoading(true)
    ...
    // Catat transaksi_stok
    await supabase.from('transaksi_stok').insert([{
      produk_id:     produkLap.id,
      tipe:          'pakai',
      sesi:          lapSesi,
      jumlah_pcs:    lapJmlPcs,
      harga_per_pcs: lapHargaFinal,
      diskon:        0,
      total:         lapTotal,
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: -0.5, marginBottom: 4 }}>🛒 Transaksi</div>
        <div style={{ fontSize: 14, color: '#94a3b8' }}>Catat penjualan dan pemakaian lapangan</div>
      </div>

      {/* Tab Navigation */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        {[
          { id: 'jual',     label: '🛒 Jual Produk' },
          { id: 'lapangan', label: '🏸 Pakai Lapangan' },
          { id: 'history',  label: '📋 History' },
        ].map(t => (
          <button key={t.id}
            style={{ padding: '8px 18px', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', border: 'none', transition: 'all 0.15s', background: tab === t.id ? '#2563eb' : '#1e293b', color: tab === t.id ? 'white' : '#94a3b8', borderBottom: tab === t.id ? '2px solid #3b82f6' : '2px solid transparent' }}
            onClick={() => setTab(t.id)}>
            {t.label}
          </button>
        ))}
              ...
              <select style={inp} value={jualProduk} onChange={e => { setJualProduk(e.target.value); setJualHarga(''); setJualJumlah('') }}>
                ...
                {produkList.map(p => (
                  <option key={p.id} value={p.id}>
                    {p.nama} — Stok: {tampilStok(p.stok_pcs, p.isi_per_satuan, p.satuan_besar, p.satuan_kecil)}
                  </option>
                ))}
              </select>
            </div>
            ...
            {produkJual && (
              <>
                ...
                <div style={{ background: '#0f172a', borderRadius: 8, padding: 14, fontSize: 13, fontFamily: 'monospace' }}>
                  ...
                  <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', color: '#94a3b8' }}>
                    ...
                    <span>Harga/{produkJual.satuan_besar}: <strong style={{ color: '#4ade80' }}>{formatRupiah(produkJual.harga_jual_besar)}</strong></span>
                    <span>Harga/{produkJual.satuan_kecil}: <strong style={{ color: '#4ade80' }}>{formatRupiah(produkJual.harga_jual_pcs)}</strong></span>
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  {/* Jumlah + Toggle Satuan */}
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                      <label style={{ ...lbl, marginBottom: 0 }}>Jumlah</label>
                      <div style={{ display: 'flex', gap: 4 }}>
                        {[
                          { v: 'besar', l: produkJual.satuan_besar },
                          { v: 'kecil', l: produkJual.satuan_kecil },
                        ].map(t => (
                          <button key={t.v}
                            style={{ padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', border: 'none', background: jualSatuan === t.v ? '#2563eb' : '#334155', color: 'white' }}
                            onClick={() => { setJualSatuan(t.v); setJualHarga('') }}>
                            {t.l}
                          </button>
                        ))}
                      </div>
                    ...
                    <input style={inp} type="number" min="1"
                      placeholder={`cth: 2 ${jualSatuan === 'besar' ? produkJual.satuan_besar : produkJual.satuan_kecil}`}
                      value={jualJumlah} onChange={e => setJualJumlah(e.target.value)} />
                    {jualJumlah > 0 && jualSatuan === 'besar' && (
                      <div style={{ marginTop: 6, fontSize: 12, color: '#4ade80', fontFamily: 'monospace' }}>
                        = {jualJmlPcs} {produkJual.satuan_kecil}
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <label style={lbl}>
                      Harga / {jualSatuan === 'besar' ? produkJual.satuan_besar : produkJual.satuan_kecil}
                      <span style={{ color: '#475569', fontWeight: 400, marginLeft: 4 }}>
                        (default: {formatRupiah(jualHargaDefault)})
                      </span>
                    </label>
                    <input style={inp} type="number"
                      placeholder={`Default: ${jualHargaDefault}`}
                      value={jualHarga} onChange={e => setJualHarga(e.target.value)} />
                  </div>
                  ...
                  <div>
                    ...
                    <input style={inp} type="number" min="0" placeholder="0"
                      value={jualDiskon} onChange={e => setJualDiskon(e.target.value)} />
                  </div>
                </div>
                ...
                {jualJumlah > 0 && (
                  <div style={{ background: '#0f172a', borderRadius: 8, padding: 14, fontSize: 13, fontFamily: 'monospace' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      ...
                      <span>{formatRupiah(jualSubtotal)}</span>
                    </div>
                    {jualDiskonNom > 0 && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                        <span style={{ color: '#94a3b8' }}>Diskon:</span>
                        <span style={{ color: '#f59e0b' }}>-{formatRupiah(jualDiskonNom)}</span>
                      </div>
                    )}
                    <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #334155', paddingTop: 8, marginTop: 4 }}>
                      <span style={{ color: '#94a3b8' }}>Total kas masuk:</span>
                      <span style={{ color: '#4ade80', fontWeight: 700, fontSize: 15 }}>{formatRupiah(jualTotal)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                      <span style={{ color: '#94a3b8' }}>Stok berkurang:</span>
                      <span style={{ color: '#dc2626' }}>-{jualJmlPcs} {produkJual.satuan_kecil}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                      <span style={{ color: '#94a3b8' }}>Sisa stok:</span>
                      <span style={{ color: produkJual.stok_pcs - jualJmlPcs < 0 ? '#dc2626' : '#f1f5f9' }}>
                        {produkJual.stok_pcs - jualJmlPcs < 0
                          ? '❌ Stok tidak cukup!'
                          : tampilStok(produkJual.stok_pcs - jualJmlPcs, produkJual.isi_per_satuan, produkJual.satuan_besar, produkJual.satuan_kecil)}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                  <button style={btnG} onClick={simpanJual} disabled={loading}>
      ...
      {tab === 'lapangan' && (
        <div style={panel}>
          ...
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {[
                  ...
                  <button key={s.v}
                    style={{ flex: 1, padding: '10px', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', border: 'none', background: lapSesi === s.v ? '#7c3aed' : '#334155', color: 'white' }}
                    onClick={() => setLapSesi(s.v)}>
                    {s.l}
                  </button>
                ))}
              </div>
            ...
            {/* Pilih Produk — filter hanya tipe shuttle */}
            <div>
              <label style={lbl}>Pilih Produk</label>
              ...
              {produkList.filter(p => p.tipe_produk === 'shuttle').length === 0 && (
                <div style={{ marginTop: 8, fontSize: 12, color: '#f59e0b' }}>
                  ⚠️ Belum ada produk bertipe Shuttle. Tambah di Manajemen Stok.
                </div>
              )}
            </div>
              ...
              <>
                <div style={{ background: '#0f172a', borderRadius: 8, padding: 14, fontSize: 13, fontFamily: 'monospace' }}>
                  ...
                  <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', color: '#94a3b8' }}>
                    ...
                    <span>Harga lapangan: <strong style={{ color: '#f59e0b' }}>{formatRupiah(produkLap.harga_pakai_pcs)}/{produkLap.satuan_kecil}</strong></span>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    ...
                    <input style={inp} type="number" min="1"
                      placeholder={`cth: 5 ${produkLap.satuan_kecil}`}
                      value={lapJumlah} onChange={e => setLapJumlah(e.target.value)} />
                  </div>
                  <div>
                    <label style={lbl}>
                      Harga / {produkLap.satuan_kecil}
                      <span style={{ color: '#475569', fontWeight: 400, marginLeft: 4 }}>(default: {formatRupiah(lapHargaDefault)})</span>
                    </label>
                    <input style={inp} type="number"
                      placeholder={`Default: ${lapHargaDefault}`}
                      value={lapHarga} onChange={e => setLapHarga(e.target.value)} />
                  </div>
                </div>
                ...
                {lapJumlah > 0 && (
                  <div style={{ background: '#0f172a', borderRadius: 8, padding: 14, fontSize: 13, fontFamily: 'monospace' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      <span style={{ color: '#94a3b8' }}>Sesi:</span>
                      <span style={{ color: '#c4b5fd' }}>{lapSesi === 'sore' ? '🌅 Sore' : '🌙 Malam'}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      <span style={{ color: '#94a3b8' }}>Pcs dipakai:</span>
                      <span style={{ color: '#dc2626' }}>-{lapJmlPcs} {produkLap.satuan_kecil}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #334155', paddingTop: 8, marginTop: 4 }}>
                      <span style={{ color: '#94a3b8' }}>Total kas masuk:</span>
                      <span style={{ color: '#4ade80', fontWeight: 700, fontSize: 15 }}>{formatRupiah(lapTotal)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                      <span style={{ color: '#94a3b8' }}>Sisa stok:</span>
                      <span style={{ color: produkLap.stok_pcs - lapJmlPcs < 0 ? '#dc2626' : '#f1f5f9' }}>
                        {produkLap.stok_pcs - lapJmlPcs < 0
                          ? '❌ Stok tidak cukup!'
                          : tampilStok(produkLap.stok_pcs - lapJmlPcs, produkLap.isi_per_satuan, produkLap.satuan_besar, produkLap.satuan_kecil)}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <button style={{ ...btnG, background: '#7c3aed' }} onClick={simpanLapangan} disabled={loading}>
      ...
      {tab === 'history' && (
        <div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            <select style={{ ...inp, width: 'auto' }} value={filterTipe} onChange={e => setFilterTipe(e.target.value)}>
              <option value="">Semua Tipe</option>
              <option value="jual">🛒 Jual</option>
              <option value="pakai">🏸 Lapangan</option>
              <option value="restock">📥 Restock</option>
            </select>
            <input style={{ ...inp, width: 'auto' }} type="date"
              value={filterTanggal} onChange={e => setFilterTanggal(e.target.value)} />
            {(filterTipe || filterTanggal) && (
              <button style={btnS} onClick={() => { setFilterTipe(''); setFilterTanggal('') }}>
                ✕ Reset Filter
              </button>
            )}
            <span style={{ marginLeft: 'auto', fontSize: 13, color: '#94a3b8', alignSelf: 'center' }}>
              {historyFiltered.length} transaksi
            </span>
          </div>
          ...
          <div style={panel}>
            {historyFiltered.length === 0 ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                <thead>
                  <tr>
                    {['Tanggal','Produk','Tipe','Jumlah','Harga','Diskon','Total','Keterangan'].map(h => (
                ...
                <tbody>
                  {historyFiltered.map(t => (
                    <tr key={t.id}>
                      ...
                      <td style={td}>
                        <span style={{ background: warnaTipe[t.tipe] || '#e2e8f0', color: teksTipe[t.tipe] || '#334155', padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 700 }}>
                      ...
                      <td style={{ ...td, fontFamily: 'monospace', color: t.diskon > 0 ? '#f59e0b' : '#475569' }}>
                        {t.diskon > 0 ? `-${formatRupiah(t.diskon)}` : '–'}
                      </td>
                      <td style={{ ...td, fontFamily: 'monospace', fontWeight: 700, color: t.tipe === 'restock' ? '#dc2626' : '#4ade80' }}>
