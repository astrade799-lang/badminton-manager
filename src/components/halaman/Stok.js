...
const inpAuto = {
  ...inp, background: '#1e293b', color: '#4ade80',
...
function ModalRestock({ item, onClose, onSelesai, tampilPesan }) {
  ...
  const hargaInput= parseInt(hargaModal) || item.harga_modal_besar || 0
  ...
  async function proses() {
    ...
    // 3. Catat pengeluaran di kas (kalau ada harga)
    if (totalBayar > 0) {
  ...
  return (
    ...
    >
      <div
        style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 12, padding: 28, width: 440, maxWidth: '90vw' }}
      ...
      >
        ...
        <div style={{ background: '#0f172a', borderRadius: 8, padding: 14, marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 20, fontSize: 13, fontFamily: 'monospace', color: '#94a3b8' }}>
            <span>Stok saat ini: <strong style={{ color: '#f1f5f9' }}>{tampilStok(item.stok_pcs, item.isi_per_satuan, item.satuan_besar, item.satuan_kecil)}</strong></span>
            <span>({item.stok_pcs} {item.satuan_kecil})</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label style={{ fontSize: 13, fontWeight: 600, color: '#94a3b8', display: 'block', marginBottom: 6 }}>
              Jumlah Restock ({item.satuan_besar})
            </label>
            <input
              style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8, color: '#f1f5f9', fontFamily: 'inherit', fontSize: 14, padding: '9px 14px', outline: 'none', width: '100%' }}
              type="number" placeholder={`cth: 5 ${item.satuan_besar}`} min="1"
              value={jumlahBesar}
              onChange={e => setJumlahBesar(e.target.value)}
              autoFocus
            />
            {jumlahBesar > 0 && (
              <div style={{ marginTop: 6, fontSize: 12, color: '#4ade80', fontFamily: 'monospace' }}>
                = {jmlPcs} {item.satuan_kecil}
              </div>
            )}
          </div>
          ...
          <div>
            <label style={{ fontSize: 13, fontWeight: 600, color: '#94a3b8', display: 'block', marginBottom: 6 }}>
              Harga Modal / {item.satuan_besar}
              <span style={{ color: '#475569', fontWeight: 400, marginLeft: 6 }}>
                (default: {formatRupiah(item.harga_modal_besar || 0)})
              </span>
            </label>
            <input
              style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8, color: '#f1f5f9', fontFamily: 'inherit', fontSize: 14, padding: '9px 14px', outline: 'none', width: '100%' }}
              type="number"
              placeholder={`Default: ${item.harga_modal_besar || 0}`}
              value={hargaModal}
              onChange={e => setHargaModal(e.target.value)}
            />
            <div style={{ marginTop: 6, fontSize: 12, color: '#94a3b8' }}>
              Kosongkan untuk pakai harga modal default
            </div>
          </div>
          ...
          {jumlahBesar > 0 && (
            <div style={{ background: '#0f172a', borderRadius: 8, padding: 14, fontSize: 13, fontFamily: 'monospace' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ color: '#94a3b8' }}>Stok bertambah:</span>
                <span style={{ color: '#4ade80' }}>+{jmlPcs} {item.satuan_kecil}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ color: '#94a3b8' }}>Stok setelah restock:</span>
                <span style={{ color: '#f1f5f9' }}>{tampilStok(item.stok_pcs + jmlPcs, item.isi_per_satuan, item.satuan_besar, item.satuan_kecil)}</span>
              </div>
              {totalBayar > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #334155', paddingTop: 8, marginTop: 8 }}>
                  <span style={{ color: '#94a3b8' }}>Kas berkurang:</span>
                  <span style={{ color: '#dc2626', fontWeight: 700 }}>-{formatRupiah(totalBayar)}</span>
                </div>
              )}
            </div>
          )}
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20 }}>
          <button
            style={{ padding: '7px 14px', borderRadius: 8, background: '#334155', color: '#f1f5f9', border: '1px solid #475569', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
            onClick={onClose}
          >
            Batal
          </button>
          <button
            style={{ padding: '7px 14px', borderRadius: 8, background: loading ? '#334155' : '#16a34a', color: 'white', border: 'none', fontSize: 13, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
            onClick={proses}
            disabled={loading}
          >
...
export default function Stok() {
  ...
  function gantiKategori(kat) {
    ...
    setForm(prev => ({
      ...prev, kategori: kat, ...d,
      harga_modal_pcs: hitungModalPcs(prev.harga_modal_besar, d.isi_per_satuan),
    }))
  }
  ...
  return (
    <div>
      ...
      {/* Modal Restock — terpisah dari toast */}
      <ModalRestock
        item={itemRestock}
        onClose={() => setItemRestock(null)}
        onSelesai={muatData}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: -0.5, marginBottom: 4 }}>📦 Manajemen Stok</div>
          <div style={{ fontSize: 14, color: '#94a3b8' }}>Kelola produk dan inventaris lapangan</div>
        </div>
          ...
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div style={{ gridColumn: 'span 2' }}>
                ...
                <input style={inp} placeholder="cth: Shuttlecock JOIN / Aqua / ECO Grip"
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {[{ v: 'shuttle', l: '🏸 Shuttle (jual + lapangan)' }, { v: 'jual', l: '🛒 Jual per pcs' }].map(t => (
            ...
            <div style={{ borderTop: '1px solid #334155', paddingTop: 16 }}>
              ...
              <div style={{ marginTop: 8, fontSize: 12, color: '#4ade80', fontFamily: 'monospace' }}>
                Preview: 1 {form.satuan_besar} = {form.isi_per_satuan} {form.satuan_kecil}
              </div>
            </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                      ...
                      <input style={inp} type="number" placeholder="cth: 115000"
                        value={form.harga_modal_besar} onChange={e => updateForm('harga_modal_besar', e.target.value)} />
                    </div>
                    <div>
                      <label style={lbl}>Harga Modal / {form.satuan_kecil} <span style={{ color: '#f59e0b', fontWeight: 400 }}>(auto-hitung)</span></label>
                      <input style={inpAuto} readOnly
                        value={modalPcsPreview > 0 ? modalPcsPreview : ''}
                        placeholder={`= ${form.harga_modal_besar || '115.000'} ÷ ${form.isi_per_satuan || '12'}`} />
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                    <div>
                      <label style={lbl}>Harga Jual / {form.satuan_besar}</label>
                      <input style={inp} type="number" placeholder="cth: 125000"
                        value={form.harga_jual_besar} onChange={e => updateForm('harga_jual_besar', e.target.value)} />
                    </div>
                    <div>
                      <label style={lbl}>Harga Jual / {form.satuan_kecil}</label>
                      <input style={inp} type="number" placeholder="cth: 14000"
                        value={form.harga_jual_pcs} onChange={e => updateForm('harga_jual_pcs', e.target.value)} />
                    </div>
                    <div>
                      <label style={lbl}>🏸 Harga Lapangan / {form.satuan_kecil}</label>
                      <input style={inp} type="number" placeholder="cth: 10000"
                        value={form.harga_pakai_pcs} onChange={e => updateForm('harga_pakai_pcs', e.target.value)} />
                    </div>
                  </div>
                  {form.harga_modal_besar > 0 && (
                    <div style={{ padding: '10px 14px', background: '#0f172a', borderRadius: 8, fontSize: 12, fontFamily: 'monospace', display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                      ...
                      {form.harga_jual_pcs   && <span><span style={{ color: '#475569' }}>Margin/{form.satuan_kecil}: </span><span style={{ color: marginPcs >= 0 ? '#4ade80' : '#dc2626' }}>{formatRupiah(marginPcs)}</span></span>}
                    </div>
                  )}
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                  <div><label style={lbl}>Harga Modal / {form.satuan_kecil}</label><input style={inp} type="number" placeholder="0" value={form.harga_modal_pcs} onChange={e => updateForm('harga_modal_pcs', e.target.value)} /></div>
                  <div><label style={lbl}>Harga Jual / {form.satuan_kecil}</label><input style={inp} type="number" placeholder="0" value={form.harga_jual_pcs} onChange={e => updateForm('harga_jual_pcs', e.target.value)} /></div>
                  <div><label style={lbl}>Harga Jual / {form.satuan_besar}</label><input style={inp} type="number" placeholder="0" value={form.harga_jual_besar} onChange={e => updateForm('harga_jual_besar', e.target.value)} /></div>
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button style={btnG} onClick={simpan}>💾 Simpan Produk</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <input style={{ ...inp, width: 'auto', flex: 1, maxWidth: 280 }} placeholder="🔍 Cari nama produk..." value={cari} onChange={e => setCari(e.target.value)} />
        <select style={{ ...inp, width: 'auto' }} value={filterKat} onChange={e => setFilterKat(e.target.value)}>
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td style={{ ...td, display: 'flex', gap: 6 }}>
                      <button
  style={{ padding: '4px 10px', borderRadius: 6, background: '#2563eb', color: 'white', border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
  onClick={() => setItemRestock(p)}
>
  📥 Restock
</button>
