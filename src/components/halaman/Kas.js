  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:isMobile?'repeat(2,1fr)':'repeat(4,1fr)', gap:isMobile?10:16, marginBottom:isMobile?16:28 }}>
        {[
          { label:'📈 Pemasukan', nilai:formatRupiah(totalMasuk),       warna:'#16a34a', sub:data.filter(t=>t.jenis==='masuk').length+' transaksi' },
          { label:'📉 Pengeluaran',nilai:formatRupiah(totalKeluar),      warna:'#dc2626', sub:data.filter(t=>t.jenis==='keluar').length+' transaksi' },
      ...
      <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap' }}>
        <input style={{ ...inp, width:'auto', flex:1, minWidth:isMobile?'100%':180, maxWidth:isMobile?'100%':260 }} placeholder="🔍 Cari keterangan..." value={cari} onChange={e=>setCari(e.target.value)} />
      ...
      <div style={panel}>
        ...
        {loading ? (
          ...
          <div>
            {dataFiltered.map(t => (
              <div key={t.id} style={{ padding:'12px 16px', borderBottom:'1px solid rgba(51,65,85,0.5)', borderLeft:`3px solid ${t.jenis==='masuk'?'#16a34a':'#dc2626'}` }}>
